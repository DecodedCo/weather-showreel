...
function getIds(city){
	$.ajax({
      ...
      success: function(data) {
      	//get historical weather data for this city id
        getHistoricalTemp(city,data.id)
      }
   });
}

function getHistoricalTemp(city,id){
	$.getJSON("http://api.openweathermap.org/data/2.5/history/city?id="+id+"&type=hour",function(data){

		tmp = []
		/*
    we will only keep 'lim' to ensure all cities have the same number of data 
    points otherwise the show reel code crashes.
    we manipulate the date so that each temperature for each 
    city is at exactly the same time otherwise bar vizs break
    */
    if (data.list.length < lim){
      console.log("Error: not enough measurements for "+city)
    }
		for (var i=0; i<Math.min(lim,data.list.length); i++){
      tmp.push({
        "symbol":city,
        "date": new Date((i+1)+" Jun 2015"), //data.list[i].dt
        "price":data.list[i].main.temp-272.15
      })
		}
...
/*
Code below from: http://bl.ocks.org/mbostock/1256572
*/
...
var m = [20, 20, 30, 20],
  ...
  w = parseInt(d3.select("#visContainer").style("width").split("px")[0]) - m[1] - m[3];
  h = parseInt(d3.select("#visContainer").style("height").split("px")[0]) - m[0] - m[2];
...
// d3.csv("stocks_short.csv", function(data) {
  ...
  symbols.forEach(function(s) {
    s.values.forEach(function(d) { 
      // console.log("date",d.date,parse(d.date))
      d.date = parse(d.date); 
      d.price = +d.price; });
...
function lines() {
  ...
  y = d3.scale.linear().range([h / nn - 20, 0]);
  ...
  var g = svg.selectAll(".symbol")
      .attr("transform", function(d, i) { return "translate(0," + (i * h / nn + 10) + ")"; });
...
function horizons() {
  ...
  g.select("circle").transition()
      .duration(duration)
      .attr("transform", function(d) { return "translate(" + (w - 60) + "," + (-h / nn) + ")"; })
      .remove();

  g.select("text").transition()
      .duration(duration)
      .attr("transform", function(d) { return "translate(" + (w - 60) + "," + (h / nn - 20) + ")"; })
      .attr("dy", "0em");
  ...
  g.each(function(d) {
    ...
    d3.select(this).selectAll(".area")
      ...
      .enter().insert("path", ".line")
        .attr("class", "area")
        .attr("transform", function(d) { return "translate(0," + (d * (h / nn - 20)) + ")"; })
